...
export class UPackCommon extends UniformPack {
	...
	setEyePos(val: Vector): void			{ this.setUniform(this.iEyePos, val); }
	setEnableClipping(val: boolean): void	{ this.setUniform(this.ibEnableClipping, val ? 1 : 0); }
	setSubspace(val: number): void			{ this.setUniform(this.iSubspace, val); }
	setbRefraction(val: boolean): void		{ this.setUniform(this.ibRefraction, val ? 1 : 0); }
	setbReflection(val: boolean): void		{ this.setUniform(this.ibReflection, val ? 1 : 0); }
	setTime(val: number): void				{ this.setUniform(this.iTime, val); }
	setMatProjView(val: Matrix): void		{ this.setUniform(this.iMatPV, val); }
