...
export class UPackWater extends UniformPack {
	constructor() {
		...
		this.iTexRefraction_Cube_ = this.addUniform({
			name: "textureRefractionCube",
			type: UniformType.INT,
			arrayLength: 1,
		});
	}

	setFoamTexSampler(val: number): void {
		this.setUniform(this.iTexFoam_, val);
	}
	setReflectionTexSampler(val: number): void {
		this.setUniform(this.iTexReflection_2D_, val);
	}
	setRefractionTexSampler(val: number): void {
		this.setUniform(this.iTexRefraction_, val);
	}
	setRefractionCubeTexSampler(val: number): void {
		this.setUniform(this.iTexRefraction_Cube_, val);
	}
	setWaterNormalTexSampler(val: number): void {
