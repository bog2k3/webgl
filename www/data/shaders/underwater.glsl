...
float approxRefractFn(float i0, float hr) {
	...
	float p = 1.3 / (r + 1.0) + pow(0.04 / hr, 2.0);
	r = pow(r, p);
...
const float ambientLightMixDistance = 1.0;
const vec3 lightHalveDist = vec3(2.0, 3.0, 4.0) * 2.0; // after how many meters of water each light component is halved
const float causticTextureTile = 0.5;

vec3 refractPos(vec3 wPos, vec3 eyePos) {
	vec3 wPos0 = vec3(wPos.x, 0.0, wPos.z);
	vec3 V0 = vec3(eyePos.x, 0.0, eyePos.z);
	...
	float fade_dist = 0.1;
	float depthFactor = clamp(-wPos.y / fade_dist, 0.0, 1.0); // fade refraction toward zero at water edges to avoid gaps
...
float computeCaustics(vec3 wPos) {
	...
	float causticIntensity = clamp(
		pow(
			dot(
				-lightDir,
				computeWaterNormal(wPos.xz * causticTextureTile, time * causticTextureTile, eyeDistForCaustic, 0.5, false)
			) * 2.0,
			5.0
		),
		0.0, 1.0
	);
	float causticSharpness = 15.0 / pow(1.0 - wPos.y, 0.9);
	causticIntensity = pow(1.0 - abs(causticIntensity - 0.3), causticSharpness);
	causticIntensity *= lightIntensity / 2.0;
...
vec3 computeLightingUnderwaterImpl(vec3 wPos, vec3 normal, float eyeDist, vec3 totalLightAtSurface) {
	...
	float eyeHeight = eyePos.y * (bReflection > 0 ? -1.0 : +1.0);

	float lightWaterDistance = wPos.y / lightDir.y;
	lightWaterDistance += eyeHeight < 0.0 ? eyeDist : 0.0;
	...
	vec3 ambientLightBelow = waterColor / pow(max(1.0, 1.0 - wPos.y), 1.5);
...
vec3 computeLightingUnderwaterSimple(vec3 wPos, vec3 normal) {
	...
	return computeLightingUnderwaterImpl(wPos, normal, 0.0, lightAtSurface);
}
...
vec3 computeWaterFog(vec3 wPos, vec3 color, float eyeDist) {
	float eyeHeight = eyePos.y * (bReflection > 0 ? -1.0 : +1.0);
	float waterThickness = eyeDist * (eyeHeight < 0.0 ? 1.0 : -wPos.y / (eyeHeight - wPos.y));
	float fogFactor = clamp(1.0 - 1.0 / (waterThickness * 0.05 + 1.0), 0.0, 1.0);
